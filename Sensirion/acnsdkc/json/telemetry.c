...
char *telemetry_serialize(arrow_device_t *device, void *d) {
  ...
  json_append_member(_node, "i|Sensirion SGP30 TVOC PPB", json_mknumber(data->tvoc_ppb));
  json_append_member(_node, "i|Sensirion SGP30 CO2 EQ PPM", json_mknumber(data->co2_eq_ppm));
  json_append_member(_node, "f|Sensirion SHTC1 Temperature", json_mknumber(data->shtc1_temperature));
  json_append_member(_node, "f|Sensirion SHTC1 Humidity", json_mknumber(data->shtc1_humidity));
#endif
